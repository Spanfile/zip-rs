...
use crate::{
    ...
    spec,
    types::{DateTime, System, ZipFileData, DEFAULT_VERSION},
};
use crc32fast::Hasher;
use podio::{LittleEndian, WritePodExt};
use std::{default::Default, io, io::prelude::*, mem};
...
impl FileOptions {
    ...
    ///
    /// The default is `CompressionMethod::Deflated`. If the deflate compression
    /// feature is disabled, `CompressionMethod::Stored` becomes the
    /// default. otherwise.
    ...
    ///
    /// The default is the current timestamp if the 'time' feature is enabled,
    /// and 1980-01-01 otherwise
...
impl<W: Write + io::Seek> ZipWriter<W> {
    ...
    ///
    /// This function ensures that the '/' path seperator is used. It also
    /// ignores all non 'Normal' Components, such as a starting '/' or '..'
    /// and '.'.
    ...
    ///
    /// This will return the writer, but one should normally not append any data
    /// to the end of the file. Note that the zipfile will also be finished
    /// on drop.
...
mod test {
    ...
    use crate::{compression::CompressionMethod, types::DateTime};
    use std::{io, io::Write};
