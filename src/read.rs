...
use std::{borrow::Cow, collections::HashMap, io, io::prelude::*};

use crate::{
    cp437::FromCp437,
    types::{DateTime, System, ZipFileData},
};
...
impl<R: Read + io::Seek> ZipArchive<R> {
    ...
    /// begins at, in bytes.
    ///
    /// Normally this value is zero, but if the zip has arbitrary data prepended
    /// to it, then this value will be the size of that prepended data.
...
impl<'a> ZipFile<'a> {
    ...
    /// Get the name of the file in a sanitized form. It truncates the name to
    /// the first NULL byte, removes a leading '/' and removes '..' parts.
...
impl<'a> Drop for ZipFile<'a> {
    fn drop(&mut self) {
        ...
        // In this case, we want to exhaust the reader so that the next file is
        // accessible.
        if let Cow::Owned(_) = self.data {
            let mut buffer = [0; 1 << 16];

            // Get the inner `Take` reader so all decompression and CRC calculation is
            // skipped.
...
///
/// This is an alternative method to read a zip file. If possible, use the
/// ZipArchive functions as some information will be missing when reading this
/// manner.
///
/// Reads a file header from the start of the stream. Will return `Ok(Some(..))`
/// if a file is present at the start of the stream. Returns `Ok(None)` if the
/// start of the central directory is encountered. No more files should be read
/// after this.
///
/// The Drop implementation of ZipFile ensures that the reader will be correctly
/// positioned after the structure is done.
///
/// Missing fields are:
